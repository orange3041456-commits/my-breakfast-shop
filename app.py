...
app = Flask(__name__)
app.secret_key = "morning_noodle_v73_stats_full_menu"
...
def boss():
    ...
    stats = {"total_count": 0, "total_money": 0, "in_count": 0, "in_money": 0, "out_count": 0, "out_money": 0, "cash_count": 0, "cash_money": 0, "line_count": 0, "line_money": 0}
    for h in history:
        if h['done']:
            stats["total_count"] += 1; stats["total_money"] += h['price']
            if "外帶" in h['loc']: stats["out_count"] += 1; stats["out_money"] += h['price']
            else: stats["in_count"] += 1; stats["in_money"] += h['price']
            if h['pay'] == "現金": stats["cash_count"] += 1; stats["cash_money"] += h['price']
            elif h['pay'] == "LINE Pay": stats["line_count"] += 1; stats["line_money"] += h['price']
...
<style>
    ...
    .header { background: #ffbe00; color: #fff; padding: 15px; text-align: center; border-radius: 0 0 15px 15px; font-weight: bold; font-size: 20px; user-select:none; }
...
<style>
    ...
    .sticky-nav { position: fixed; top: 0; left: 0; right: 0; z-index: 1000; background: #333; padding: 10px; }
    ...
    .btn{padding:12px;border:none;border-radius:5px;font-weight:bold;cursor:pointer;margin-right:8px;}
    ...
    @media print { body * { visibility: hidden; } #print-area, #print-area * { visibility: visible; } #print-area { position: absolute; left: 0; top: 0; width: 100%; font-size: 20px; line-height: 1.5; } .no-print { display: none !important; } }
</style>
<script>
...
<body>
    ...
    <div class="stats-container no-print">
        <div style="font-weight:bold;">📈 今日營運概況 (已結帳)</div>
        <div class="stats-grid">
            <div class="stat-card" style="grid-column: span 2;">總營收: <span class="stat-val">${{stats.total_money}}</span> ({{stats.total_count}}單)</div>
            <div class="stat-card">🏠 內用: ${{stats.in_money}}</div><div class="stat-card">🥡 外帶: ${{stats.out_money}}</div>
            <div class="stat-card">💵 現金: ${{stats.cash_money}}</div><div class="stat-card">📲 LINE: ${{stats.line_money}}</div>
        </div>
    </div>
    {% for h in logs %}<div class="o {{ 'done' if h.done else '' }}">
        ...
        <div class="no-print">
            {% if not h.done %}
                <button class="btn cash" onclick="payAndPrint('{{h.id}}','現金','{{h.loc}}','{{h.time.strftime('%m/%d %H:%M:%S')}}','{{h.summary|safe}}','{{h.price}}')">現金</button>
            ...
            {% else %}<div style="display:flex;justify-content:space-between;align-items:center;"><span>✅ ({{h.pay}})</span><button class="btn reset" onclick="payAndPrint('{{h.id}}','RESET')">重設</button></div>{% endif %}
        </div>
    </div>{% endfor %}
</body></html>
"""
...
if __name__ == "__main__":
    app.run(host='0.0.0.0', port=int(os.environ.get("PORT", 10000)))    session['cart'] = [i for i in session.get('cart', []) if i['id'] != request.form.get("id")]
    return jsonify({"status": "ok"})
...
<style>
    body { font-family: sans-serif; background: #fdfaf0; margin: 0; padding: 0 10px 100px; }
    .header { background: #ffbe00; color: #fff; padding: 15px; text-align: center; border-radius: 0 0 15px 15px; font-weight: bold; font-size: 20px; user-select: none; -webkit-user-select: none; }
    .setup { background: #fff; margin: 10px 0; padding: 12px; border-radius: 10px; border-left: 6px solid #ffbe00; }
    .btn { padding: 8px 15px; border: 1.5px solid #ddd; border-radius: 20px; background: #f8f9fa; margin: 3px; cursor: pointer; }
    .btn.active { background: #ffbe00; font-weight: bold; border-color: #ffbe00; }
    .title { background: #5d4037; color: #fff; padding: 10px 15px; border-radius: 5px; margin-top: 15px; }
